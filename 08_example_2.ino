...
// turn on the LED if the distance is between dist_min and dist_max
  if (dist_min <= dist_raw & dist_raw <= 200) {
    analogWrite(PIN_LED, 255-(255/100*(dist_raw-100)));
  }

  else if (200 < dist_raw & dist_raw <= dist_max) {
    analogWrite(PIN_LED, 255-(255/100*(300-dist_raw)));
  }
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min || reading > dist_max) reading = a; // return 0 when out of range.
  else a = reading;
  return reading;
